...
export default function LandingPage() {
  ...
  useEffect(() => {
    const pendingEmail = window.sessionStorage.getItem("twillerPendingLoginOtp");
    if (pendingEmail) {
  ...
  const handleSocialLogin = async (signIn: () => Promise<void>) => {
    try {
      await signIn();
    } catch (error: any) {
      if (error.message === "EMAIL_OTP_REQUIRED") {
        const email = error.otpEmail || "";
        window.sessionStorage.setItem("twillerPendingLoginOtp", email);
        setOtpEmail(email);
        setAuthMode("otp");
        setShowAuthModal(true);
      } else {
        ...
        <div className="space-y-12">
          ...
          <div className="space-y-4 max-w-xs">
            ...
            <Button
              variant="outline"
              className="w-full py-3 rounded-full border-gray-600  bg-black text-white font-semibold text-base h-12"
             onClick={() => handleSocialLogin(applesignin)}
            >
              <svg
