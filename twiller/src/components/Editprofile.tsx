...
const Editprofile = ({ isopen, onclose }: any) => {
  ...
  const handlePhotoUpload = async (
    ...
    field: "avatar" | "coverPhoto" = "avatar"
  ) => {
    ...
    try {
      ...
      setError({ general: error.response?.data?.error || error.message || "Image upload failed." });
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <CardHeader className="relative pb-4 border-b border-gray-800">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <CardTitle className="text-xl font-bold">{translate(lang, "editProfile")}</CardTitle>
            </div>
            <Button
              type="submit"
            ...
            >
              {isLoading ? (
                <div className="flex items-center space-x-2">
                  ...
                  <span>{translate(lang, "saving")}</span>
                </div>
              ) : (
                translate(lang, "save")
              )}
